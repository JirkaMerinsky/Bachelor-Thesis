...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="lst:invoice">
        ...
        <xsl:variable name="agendaType">PRIJEM</xsl:variable>
        ...
        <faktura-prijata>
            ...
            <xsl:choose>
                <xsl:when test="inv:invoiceDetail">
                    ...
                    <xsl:sequence select="
                            ic:generateAccountingForInvoiceFromAccountingId($accountingId,
                            $account-assignment_flexi, $agendaType)"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="inv:invoiceItem">
        ...
        <faktura-prijata-polozka>
            ...
            <xsl:sequence select="
                    ic:generateAccountingForInvoiceItem($documentNumber, $documentId, current(),
                    $agendaType, $journal_pohoda, $agendaId)"/>
        </faktura-prijata-polozka>
    </xsl:template>
