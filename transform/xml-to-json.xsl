...
<xsl:stylesheet version="2.0"
  ...
  <xsl:param name="ignore-null-fields" as="xs:boolean" select="true()"/>
  ...
  <xsl:function name="json:generate" as="xs:string">
    ...
    <xsl:variable name="json-tree">
      <json:object>
        <xsl:copy-of select="if (not($use-rayfish)) then json:create-node-attr($xml/child::node(), false()) else json:create-simple-node($xml/child::node())"/>
      </json:object>
    </xsl:variable>
  ...
  <xsl:template name="json:build-tree">
    <xsl:param name="input" as="node()"/>
    <json:object>
      <xsl:copy-of select="if (not($use-rayfish)) then json:create-node-attr($input, false()) else json:create-simple-node($input/child::node())"/>
    </json:object>
  </xsl:template>
          ...
          <xsl:for-each-group select="$node/child::node()" group-adjacent="local-name()">
            <xsl:choose>
              <xsl:when test="count(current-group()) eq 1 and (not(exists(./@json:force-array)) or ./@json:force-array eq 'false')">
                <xsl:copy-of select="json:create-node-attr(current-group()[1], false())"/>
              </xsl:when>
              <xsl:otherwise>
                <!-- TODO: beautify -->
                <xsl:if test="count(./@*) gt 0">
                  <xsl:copy-of select="json:create-attributes(.)"/>
                </xsl:if>
                <json:member>
        ...
        <json:array-value>
          <json:value>
            <xsl:choose>
              ...
              <xsl:otherwise>
                <json:object>
                  <xsl:copy-of select="json:create-node-attr(.,false())"/>
                </json:object>
              </xsl:otherwise>
            </xsl:choose>
          </json:value>
  ...
  <xsl:function name="json:create-attributes" as="node()*">
    ...
    <xsl:for-each select="$node/@*[not(../@json:force-array) or count(.|../@json:force-array)=2]">
      <json:member>
        <json:name><xsl:if test="$use-badgerfish or $use-rabbitfish"><xsl:value-of select="name(..)" />@</xsl:if><xsl:value-of select="if($use-namespaces) then name() else local-name()"/></json:name>
