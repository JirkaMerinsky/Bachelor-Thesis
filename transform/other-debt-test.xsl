...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="lst:invoice">
        ...
        <xsl:param name="agendaType">VYDEJ</xsl:param>
        <xsl:param name="invoiceType">code:OST. POHLEDÁVKY</xsl:param>
    ...
    <xsl:template match="inv:invoiceItem">
        ...
        <pohledavka-polozka>
            <xsl:sequence
                select="ic:generateCommonInvoiceItemWithoutAccounting($year, current(), false())"/>
            <xsl:sequence select="
